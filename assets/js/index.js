...
(async () => {
  ...
  try {
    ...
    // Удобная безопасная функция
    const toInt = (v) => {
      const n = Number(v);
      return Number.isFinite(n) ? n.toLocaleString('en-US') : '0';
    };

    // KPI: проекты / ссылки / клики
    const s = await API.summary();
    setNum('#kpi-projects', toInt(s?.projects));
    setNum('#kpi-links',    toInt(s?.links));
    setNum('#kpi-clicks',   toInt(s?.clicks));

    // KPI: уникальные пользователи (все ссылки)
    try {
      const resp = await fetch('/api/stats/project/', { credentials: 'same-origin' });
